...
describe("RefundRuleFactory", () => {
    ...
    // testes de borda para garantir que PartialRefund é retornado corretamente
    it("deve retornar PartialRefund caso seja exatamente 7 dias antes do check-in", () => {
        const rule = RefundRuleFactory.getRefundRule(7);
        expect(rule).toBeInstanceOf(PartialRefund);
    });

    it("deve retornar PartialRefund caso seja exatamente 1 dia antes do check-in", () => {
        const rule = RefundRuleFactory.getRefundRule(1);
        expect(rule).toBeInstanceOf(PartialRefund);
    });
});